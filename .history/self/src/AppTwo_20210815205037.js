...
function Component_Fetch_Data_With_API() {
  ...
  const [stateLoading, setLoading] = useState(null);
  const [stateError, setError] = useState(null);

  debugger;

  console.log(get_stateLoading());
  ...
  useEffect(() => {
    func_fetdata_retreiverch_data(URL + login, setData, setLoading, setError);
  }, []);
  ...
  return (
    <div>
      ...
      <p>{get_stateLoading()}</p>
      <p>{stateData ? "stateData has something" : "data is null"}</p>
      <p>{stateError ? stateError : "no error"}</p>
    </div>
  );
