...
const cb_fetchUser_2 = (stateUserID)=
//start fetch

  //store userID in userID
  .then((user) => set_stateUserID(user));

  const cb_CompUser = (props) => {
    //pull userID from the URL link
    const { userID_in_url } = props.match.params;
    set_stateUserID(userID_in_url);
  ...
  return (
    <div>
      ...
      <BrowserRouter>
        ...
        <Route path="/userprofile/users/:id" render={cb_CompUser} />
      </BrowserRouter>
    </div>
  );
}
